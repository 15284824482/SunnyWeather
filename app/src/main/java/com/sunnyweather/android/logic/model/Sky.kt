...
/*
   因为彩云天气返回的数据中,天气情况都是一些诸如CLOUDY,WIND之类的天气代码,需要编写一个转换函数将这些
   天气代码转换成一个Sky对象.
   这里首先定义了一个Sky类作为数据模型,它包含了info,icon,bg这3个字段,分别表示该天气情况对应的文字,图
   标和背景.然后使用mapOf()函数来定义每种天气代码所应该对应的文字,图标和背景.最后,定义一个getSky()方
   法来根据天气代码获取对应的Sky对象
*/
